...
function renderFindingRankingTable(rows) {
  findingRankingTableBody.innerHTML = "";

  if (!rows.length) {
    findingRankingTableBody.innerHTML = '<tr><td colspan="2" class="text-muted">Sin datos</td></tr>';
    return;
  }
  ...
  rows.forEach((row) => {
    ...
    tr.innerHTML = `
      <td>${row.finding || "Hallazgo sin detalle"}</td>
      <td>${row.hallazgos ?? "-"}</td>
    `;
